...
class CronService:
    ...
    def __init__(
        self,
        store_path: Path = None,  # Kept for backward compatibility, not used
    ...
    def _load_store(self) -> CronStore:
        ...
        try:
            ...
            logger.warning(f"Failed to load cron store from MongoDB: {e}")
    ...
    def _save_store(self) -> None:
        ...
        if not self._store:
            return
        # Remove all jobs and re-insert (simple approach)
    ...
    async def start(self) -> None:
        ...
        # # Execute any jobs that are already due (next_run_at_ms <= now)
        # if self._store:
        #     now = _now_ms()
